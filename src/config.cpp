...
std::wstring Config::DefaultPath() {
    ...
    if (SUCCEEDED(SHGetKnownFolderPath(FOLDERID_RoamingAppData, 0, nullptr, &appdata))) {
        ...
        dir = roaming + L"\\DualCursor";
        CreateDirectoryW(dir.c_str(), nullptr);
        const std::wstring path = dir + L"\\dualcursor.ini";
        ...
        if (GetFileAttributesW(path.c_str()) == INVALID_FILE_ATTRIBUTES) {
            const std::wstring openMouseConfig = roaming + L"\\openMouse\\openmouse.ini";
            const std::wstring legacy = roaming + L"\\OpenMPX\\openmpx.ini";
            if (GetFileAttributesW(openMouseConfig.c_str()) != INVALID_FILE_ATTRIBUTES)
                CopyFileW(openMouseConfig.c_str(), path.c_str(), TRUE);
            else if (GetFileAttributesW(legacy.c_str()) != INVALID_FILE_ATTRIBUTES)
...
bool Config::Save(const std::wstring& path) const {
    std::wstring t;
    t += L"; DualCursor configuration\n";
    t += L"; Device keys are raw input interface paths. Run DualCursor.exe --list\n";
...
void OpenLogFile() {
    ...
    path = (slash == std::wstring::npos) ? std::wstring(L"dualcursor.log")
                                         : path.substr(0, slash + 1) + L"dualcursor.log";
